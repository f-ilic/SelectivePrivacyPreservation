...
for dataset in datasets:
    base_dir = f"runs/privacy/{dataset}"
    ...
    for tbfile in tbfiles:
        ...
        experiment = tbfile.replace(base_dir, "").split("/")[1]
        experiment = experiment.replace(
            "____pretrained__True____train_backbone__True", ""
        )
        experiment = experiment.replace(
            "____pretrained__True____train_backbone__False", ""
        )
        ...
        # experiment = experiment.replace("interpolation maskednearest", "")
        ...
        if not os.path.exists(bestepoch):
            for i in np.arange(1, 100, 1):
                bestepoch = (
                    "/".join(tbfile.split("/")[:-2])
                    + f"/models/checkpoint_epoch{t.step+i}.pt"
                )
                if os.path.exists(bestepoch):
                    break

        print(f"{dataset} \t {name} \t {experiment} \t {t.value:.2f}")
        if os.path.exists(bestepoch):
