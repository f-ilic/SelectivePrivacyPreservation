...
if __name__ == "__main__":
    ...
    all_actions = torch.stack(
        [torch.Tensor(ipn_action), torch.Tensor(kth_action), torch.Tensor(sbu_action)]
    )
    all_actions = all_actions.mean(0).tolist()

    all_privacy = torch.stack(
        [
            torch.Tensor(ipn_privacy),
            torch.Tensor(kth_privacy),
            torch.Tensor(sbu_privacy),
        ]
    )
    all_privacy = all_privacy.mean(0).tolist()

    for lbl, a, p in zip(labels, all_actions, all_privacy):
        if lbl in [
            "ELR\cite{ryoo2018extremelowres} s=2",
            "ELR\cite{ryoo2018extremelowres} s=4",
            "ELR\cite{ryoo2018extremelowres} s=8",
