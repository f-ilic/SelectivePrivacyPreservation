...
def read_flow(path):
    if not isinstance(path, io.BufferedReader):
        ...
        if not path.split(".")[-1] == "flo":
            raise AssertionError(
                "File extension [flo] required, [{f}] given".format(
                    f=path.split(".")[-1]
                )
            )
    ...
    nbands = 2
    tmp = np.frombuffer(
        flo.read(nbands * width * height * 4), np.float32, count=nbands * width * height
    )
...
def _compute_color(u, v):
    ...
    idxNans = np.where(np.logical_or(np.isnan(u), np.isnan(v)))
    u[idxNans] = 0
    v[idxNans] = 0
...
def video_normalized_flow(flow):  # T H W 2
    t = flow.shape[0]
    flow = flow.numpy()
    maxrad = -1
    for i in range(t):
    ...
    return flow / (maxrad + eps)


def video_flow2color(flow):
    imgs = []
    t = flow.shape[0]
    for i in range(t):
        imgs.append(_compute_color(flow[i, :, :, 0], flow[i, :, :, 1]))
...
def _normalize_flow(flow):
    ...
    if not nBands == 2:
        raise AssertionError(
            "Image must have two bands. [{h},{w},{nb}] shape given instead".format(
    ...
    # Fix unknown flow
    idxUnknown = np.where(
        np.logical_or(abs(u) > UNKNOWN_FLOW_THRESH, abs(v) > UNKNOWN_FLOW_THRESH)
    )
    u[idxUnknown] = 0
    v[idxUnknown] = 0
